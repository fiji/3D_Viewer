...
public class SmoothControl {
	...
	static private final void apply(final CustomTriangleMesh m,
		final List<Point3f> triangles)
	{
		final List<Point3f> current = m.getMesh(); // the triangles, specified as
	...
	static private final class Originals {
		...
		final private Map<CustomTriangleMesh, List<Point3f>> data =
			new HashMap<CustomTriangleMesh, List<Point3f>>();
		...
		private void restore(final Content except) {
			...
			for (final Map.Entry<CustomTriangleMesh, List<Point3f>> e : data
				.entrySet())
			{
		...
		private void copy(final Image3DUniverse univ, final boolean all) {
			if (all) for (final Content c : (Collection<Content>) univ.getContents())
				add(c);
			else add(univ.getSelected());
		}
		...
		/**
		 * Add any CustomTriangleMesh contained in
		 * 
		 * @param content only if not there already.
		 */
	...
	{
		final HashSet<CustomTriangleMesh> meshes =
			new HashSet<CustomTriangleMesh>();
		if (null == content) return meshes;
		final ContentInstant ci = content.getCurrent();
		if (null == ci) return meshes;
		final ContentNode node = ci.getContent();
		...
		if (node instanceof CustomMultiMesh) {
			...
			final CustomMesh m = ((CustomMeshNode) node).getMesh();
			if (m instanceof CustomTriangleMesh) {
				meshes.add((CustomTriangleMesh) m);
			}
		}
		else if (node instanceof MeshGroup) {
			final CustomMesh m = ((MeshGroup) node).getMesh();
			if (m instanceof CustomTriangleMesh) {
				meshes.add((CustomTriangleMesh) m);
			}
		}
	...
	static private final void smooth(final CustomTriangleMesh tm,
		final int iterations, final Originals originals)
	{
		...
		final Originals originals)
	{
		...
		for (final CustomTriangleMesh tm : meshes) {
	...
	static private Task launchTask(final GenericDialog gd, final Scrollbar s,
		final Choice c, final Object pivot, final Task[] state, final Runnable r)
	{
	...
	public SmoothControl(final Image3DUniverse univ) {
		...
		final GenericDialog gd = new GenericDialog("Smooth meshes") {
			...
			public void windowClosed(final WindowEvent we) {
				final GenericDialog gd = (GenericDialog) we.getSource();
				if (gd.wasCanceled()) {
			...
			private final Task task(final int iterations) {
				...
				return launchTask(gd, slider, choice, pivot, lastTask, new Runnable() {
					...
					public void run() {
						...
						if (1 == choiceValue) {
							// All meshes
							for (final Content c : (Collection<Content>) univ.getContents()) {
								if (0 == all[0] && 1 == choiceValue && c == univ.getSelected()) continue; // skip
																																													// selected
																																													// when
																																													// going
																																													// from
