...
class VectorString3D {
	...
	public VectorString3D(final double[] x, final double[] y, final double[] z) {
		if (!(x.length == y.length && x.length == z.length)) throw new RuntimeException(
	...
	/**
	 * Add an array that will get resampled along; must be of the same length as
	 * the value returned by length()
	 */
	public void addDependent(final double[] a) throws Exception {
		if (a.length != this.length) throw new Exception(
			"Dependent array must be of the same size as thevalue returned by length()");
		if (null == dep) {
			dep = new double[1][];
			dep[0] = a;
		}
	...
	/**
	 * The length of this string, that is, the number of points (and vectors) in
	 * it.
	 */
	...
	private static class ResamplingData {
		...
		private double[] rx, ry, rz, vx, vy, vz;
		private double[][] dep;
		...
		final void setP(final int i, final double xval, final double yval,
			final double zval)
		{
		...
		/** Arrays are enlarged if necessary. */
		final void setV(final int i, final double xval, final double yval,
			final double zval)
		{
			if (i >= rx.length) resize(i + 10);
			this.vx[i] = xval;
		...
		/** Arrays are enlarged if necessary. */
		final void setPV(final int i, final double rxval, final double ryval,
			final double rzval, final double xval, final double yval,
			final double zval)
		{
			if (i >= rx.length) resize(i + 10);
			this.rx[i] = rxval;
		...
		final void resize(final int new_length) {
			...
			if (null != dep) {
				...
				for (int i = 0; i < dep.length; i++)
					dep2[i] = Utils.copy(dep[i], new_length);
				dep = dep2;
			}
		}

		final double x(final int i) {
			return rx[i];
		}

		final double y(final int i) {
			return ry[i];
		}

		final double z(final int i) {
			return rz[i];
		}

		/** Distance from point rx[i],ry[i], rz[i] to point x[j],y[j],z[j] */
		final double distance(final int i, final double x, final double y,
			final double z)
		{
			return Math.sqrt(Math.pow(x - rx[i], 2) + Math.pow(y - ry[i], 2) +
				Math.pow(z - rz[i], 2));
		}
		...
		final void put(final VectorString3D vs, final int length) {
			...
			if (null != dep) {
				...
				for (int i = 0; i < dep.length; i++)
					vs.dep[i] = Utils.copy(dep[i], length);
			}
		}

		final void setDeps(final int i, final double[][] src_dep,
			final int[] ahead, final double[] weight, final int len)
		{
			if (null == dep) return;
			if (i >= rx.length) resize(i + 10);
			//
			...
			} // above, the ahead and weight arrays (which have the same length) could
				// be of larger length than the given 'len', thus len is used.
		}
	}
	...
	private static class Vector {
		...
		final void put(final int i, final ResamplingData r) {
			r.setPV(i, r.x(i - 1) + this.x, r.y(i - 1) + this.y, r.z(i - 1) + this.z,
		...
		final void put(final int i, final double[] x, final double[] y,
			final double[] z)
		{
			x[i] = this.x;
			y[i] = this.y;
		...
		final void changeRef(final Vector v_delta, final Vector v_i1,
			final Vector v_new1)
		{ // this vector works like new2
			...
			final Matrix mthis =
				new Matrix(new double[] { this.x, this.y, this.z }, 1);
			// The rotated vector as a one-dim matrix
			// (i.e. the rescued difference vector as a one-dimensional matrix)
			final Matrix v_rot = R.transpose().times(mthis.transpose()); // 3x3 times
																																		// 3x1,
																																		// hence the
																																		// transposing
																																		// of the
	...
	private final void recalculate(final double[] w, final int length,
		final double sum_)
	{
		double sum = 0;
		int q;
	...
	private void resample() {
		...
		// the first vector is 0,0,0 unless the path is closed, in which case it
		// contains the vector from last-to-first.
		...
		try {
			...
			for (; prev_i <= i;) {
				...
				if (0 == next_ahead) {
					...
					// Just use the next point as target towards which create a vector of
					// length delta
					vector.set(x[i] - r.x(j - 1), y[i] - r.y(j - 1), z[i] - r.z(j - 1));
					dist1 = vector.length();
					vector.setLength(delta);
					vector.put(j, r);
					if (null != dep) r.setDeps(j, dep, new int[] { i },
						new double[] { 1.0 }, 1);

					// System.out.println("j: " + j + " (ZERO)  " + vector.computeLength()
					// + "  " + vector.length());

					// correct for point overtaking the not-close-enough point ahead in
					// terms of 'delta_p' as it is represented in MAX_DISTANCE, but
					// overtaken by the 'delta' used for subsampling:
					if (dist1 <= delta) {
						// look for a point ahead that is over distance delta from the
						// previous j, so that it will lay ahead of the current j
						for (u = i; u < this.length; u++) {
							dist2 =
								Math.sqrt(Math.pow(x[u] - r.x(j - 1), 2) +
					...
					for (u = 0; u < next_ahead; u++) {
						...
						ve[u].set(x[iu] - r.x(j - 1), y[iu] - r.y(j - 1), z[iu] -
							r.z(j - 1));
						ve[u].setLength(w[u] * delta);
						vector.add(ve[u], u == next_ahead - 1); // compute the length only
					...
					// System.out.println("j: " + j + "  (" + next_ahead + ")   " +
					// vector.computeLength() + "  " + vector.length());

					// find the first point that is right ahead of the newly added point
					// so: loop through points that lay within MAX_DISTANCE, and find the
					// first one that is right past delta.
					ii = i;
					...
					// correct for the case of unseen point (because all MAX_POINTS ahead
					// lay under delta):
					prev_i = i;
					if (i == ii) {
						i = ahead[next_ahead - 1] + 1; // the one after the last.
						if (i >= this.length) {
							if (isClosed()) i = i - this.length; // this.length is the length
																										// of the x,y,z, the
																										// original points
			...
			e.printStackTrace();
			System.out.println("Some data: x,y,z .length = " + x.length + "," +
				y.length + "," + z.length + "\nj=" + j + ", i=" + i + ", prev_i=" +
				prev_i);
		}

		dist_ahead =
			r.distance(j - 1, x[this.length - 1], y[this.length - 1],
				z[this.length - 1]);

		// System.out.println("delta: " + delta + "\nlast point: " + x[x.length-1] +
		// ", " + y[y.length-1] + ", " + z[z.length-1]);
		// System.out.println("last resampled point: x,y,z " + r.x(j-1) + ", " +
		// r.y(j-1) + ", " + r.z(j-1));
		// System.out.println("distance: " + dist_ahead);

		// see whether the subsampling terminated too early, and fill with a line of
		// points.
		...
		if (dist_ahead > delta * 1.2) {
			...
			while (dist_ahead > delta * 1.2) {
				...
				vector.set(x[last_i] - r.x(j - 1), y[last_i] - r.y(j - 1), z[last_i] -
					r.z(j - 1));
		...
		// vector at zero is left as 0,0 which makes no sense. Should be the last
		// point that has no vector, or has it only in the event that the list of
		// points is declared as closed: a vector to the first point. Doesn't really
		// matter though, as long as it's clear: as of right now, the first point
		// has no vector unless the path is closed, in which case it contains the
		// vector from the last-to-first.
	}
}
