...
public class Pipe {
	...
	static public List<Point3f> generateTriangles(final double[][][] all_points,
		final double scale)
	{
	 ...
	 * third dimension is the x,y,z of each vertex. pointColorList is input,
	 * telling this method what the colour of each point along the path is, while
	 * vertexColorList should be an empty list used for output - the color of each
	 * vertex in the triangulation will be returned in there.
	 */

	static public List<Point3f> generateTriangles(final double[][][] all_points,
		final double scale, final List<Color3f> pointColorList,
		final List<Color3f> vertexColorList)
	{
		final boolean outputColors =
			(pointColorList != null) && (vertexColorList != null);
		if (outputColors && vertexColorList.size() > 0) throw new RuntimeException(
			"vertexColorList in Pipe.generateTriangles() is only for output: should be empty");

		final int n = all_points.length;
		final int parallels = all_points[0].length - 1;
		final List<Point3f> list = new ArrayList<Point3f>();
		for (int i = 0; i < n - 1; i++) { // minus one since last is made with
																			// previous
			for (int j = 0; j < parallels; j++) { // there are 12+12 triangles for
																						// each joint //it's up to 12+1
																						// because first point is repeated
																						// at the end
				// first triangle in the quad
				list.add(new Point3f((float) (all_points[i][j][0] * scale),
					(float) (all_points[i][j][1] * scale),
					(float) (all_points[i][j][2] * scale)));
				list.add(new Point3f((float) (all_points[i][j + 1][0] * scale),
					(float) (all_points[i][j + 1][1] * scale),
					(float) (all_points[i][j + 1][2] * scale)));
				list.add(new Point3f((float) (all_points[i + 1][j][0] * scale),
					(float) (all_points[i + 1][j][1] * scale),
					(float) (all_points[i + 1][j][2] * scale)));
				...
				list.add(new Point3f((float) (all_points[i][j + 1][0] * scale),
					(float) (all_points[i][j + 1][1] * scale),
					(float) (all_points[i][j + 1][2] * scale)));
				list.add(new Point3f((float) (all_points[i + 1][j + 1][0] * scale),
					(float) (all_points[i + 1][j + 1][1] * scale),
					(float) (all_points[i + 1][j + 1][2] * scale)));
	...
	static public double[][][] makeTube(double[] px, double[] py, double[] pz,
		double[] p_width_i, final int resample, final int parallels,
		final boolean do_resample, final Color3f flatColor,
		final ImagePlus colorImage, final List<Color3f> outputColors)
	{

		final boolean colorsSpecified = flatColor != null || colorImage != null;
		if (colorsSpecified) {
			if (outputColors == null) throw new RuntimeException(
				"If you specify flatColor or colorImage in a call to makeTube(), then outputColors must be non-null");
			if (outputColors.size() > 0) throw new RuntimeException(
				"The outputColors list in makeTube should be empty; it's for output, not input");
		}
		...
		if (do_resample) {
			try {
				final VectorString3D vs = new VectorString3D(px, py, pz);
				...
				// Utils.log("lengths:  " + px.length + ", " + py.length + ", " +
				// pz.length + ", " + p_width_i.length);
				n = vs.length();
			}
		...
		if (colorsSpecified) {
			final Color3f[] finalColors =
				getPointColors(px, py, pz, flatColor, colorImage);
		...
		final Vector3 betweenPoints = new Vector3();
		Vector3 lastFirstSpoke = null;
		final Vector3[] circle = new Vector3[parallels + 1];
		final Vector3 intersection = new Vector3();
		final Vector3 crossForOffset = new Vector3();

		// Pre-compute the sin and cos of each angle:
		final double[] sinn = new double[parallels];
		final double[] cosn = new double[parallels];
		...
		final Vector3 unit_y = new Vector3(0, 1, 0);
		final Vector3 unit_z = new Vector3(0, 0, 1);

		for (int i = 0; i < n; i++) {

			// The vector from point i to the next one:
			if (i < n - 1) betweenPoints.set(px[i + 1] - px[i], py[i + 1] - py[i],
				pz[i + 1] - pz[i]);
			else {
			...
			if (betweenPoints.isZero(epsilon)) throw new RuntimeException(
				"Two points on the path were the same");
			 ...
			 */
			final int t1_a = Math.max(0, i - 1);
			final int t1_b = Math.min(n - 1, i + 1);
			final int t2_a = Math.min(n - 2, i);
			final int t2_b = Math.min(n - 1, i + 2);

			final Vector3 tangent1 =
				new Vector3(px[t1_b] - px[t1_a], py[t1_b] - py[t1_a], pz[t1_b] -
					pz[t1_a]);
			final Vector3 tangent2 =
				new Vector3(px[t2_b] - px[t2_a], py[t2_b] - py[t2_a], pz[t2_b] -
					pz[t2_a]);
			...
			for (int q = 0; q < parallels; ++q) {
				final int a = (q + offset) % parallels;
				circle[q] =
					rotate_v_around_axis(intersection, tangent1, sinn[a], cosn[a]);
			}
	...
	static private Vector3 rotate_v_around_axis(final Vector3 v,
		final Vector3 axis, final double sin, final double cos)
	{

		final Vector3 result = new Vector3();
		final Vector3 r = axis.normalize(axis);

		result.set((cos + (1 - cos) * r.x * r.x) * v.x +
			((1 - cos) * r.x * r.y - r.z * sin) * v.y +
			((1 - cos) * r.x * r.z + r.y * sin) * v.z, ((1 - cos) * r.x * r.y + r.z *
			sin) *
			v.x +
			(cos + (1 - cos) * r.y * r.y) *
			v.y +
			((1 - cos) * r.y * r.z - r.x * sin) * v.z, ((1 - cos) * r.y * r.z - r.y *
			sin) *
			v.x +
			((1 - cos) * r.y * r.z + r.x * sin) *
			v.y +
			(cos + (1 - cos) * r.z * r.z) * v.z);
	...
	public static final int enrangeInteger(final float value, final int mininum,
		final int maximum)
	{
		return Math.max(mininum, Math.min(maximum, Math.round(value)));
	}

	public static final int enrangeInteger(final double value, final int mininum,
		final int maximum)
	{
		return Math.max(mininum, Math.min(maximum, (int) Math.round(value)));
	}

	public static Color3f[] getPointColors(final double[] x_points,
		final double[] y_points, final double[] z_points, final Color3f flatColor,
		final ImagePlus colorImage)
	{
		final int n = x_points.length;
		final Color3f[] result = new Color3f[x_points.length];
		Arrays.fill(result, flatColor);
		if (colorImage != null) {
			final ImageStack stack = colorImage.getStack();
			final int width = colorImage.getWidth();
			final int height = colorImage.getHeight();
			final int depth = colorImage.getStackSize();
			final int type = colorImage.getType();

			double x_spacing = 1;
			double y_spacing = 1;
			double z_spacing = 1;
			final Calibration calibration = colorImage.getCalibration();
			if (calibration != null) {
			...
			final java.awt.image.ColorModel cm =
			...
			if (cm != null && cm instanceof IndexColorModel) {
				final java.awt.image.IndexColorModel icm = (IndexColorModel) cm;
			...
			for (int i = 0; i < n; ++i) {
				...
				switch (type) {
					...
					case ImagePlus.GRAY8: {
						...
						final int v = bp.getPixel(x, y);
						if (cm == null) {
							final float fv = v / 255.0f;
							result[i] = new Color3f(fv, fv, fv);
						}
						else {
							result[i] =
								new Color3f((reds[v] & 0xFF) / 255.0f,
									(greens[v] & 0xFF) / 255.0f, (blues[v] & 0xFF) / 255.0f);
						}
					}
						break;
					...
					case ImagePlus.GRAY16: {
						final ShortProcessor sp =
							(ShortProcessor) stack.getProcessor(z + 1);
						final int s = sp.getPixel(x, y);
						final int v =
							enrangeInteger((s - sp.getMin()) * 255.0f /
								(sp.getMax() - sp.getMin()), 0, 255);
						result[i] =
							new Color3f((reds[v] & 0xFF) / 255.0f,
								(greens[v] & 0xFF) / 255.0f, (blues[v] & 0xFF) / 255.0f);
					}
					...
					case ImagePlus.GRAY32: {
						final FloatProcessor fp =
							(FloatProcessor) stack.getProcessor(z + 1);
						final float f = fp.getf(x, y);
						final int v =
							enrangeInteger((f - fp.getMin()) * 255 /
								(fp.getMax() - fp.getMin()), 0, 255);
						result[i] =
							new Color3f((reds[v] & 0xFF) / 255.0f,
								(greens[v] & 0xFF) / 255.0f, (blues[v] & 0xFF) / 255.0f);
					}
					...
					default:
						throw new RuntimeException("colorImage type: " + type +
